...
func (r *SegmentsRepo) Add(ctx context.Context, segment *repository.Segment) error {
	var title string
	err := r.db.ExecQueryRow(ctx, "INSERT INTO segments(title, desc, created_at, updated_at) VALUES ($1, $2, $3, $3) RETURNING title", segment.Title, segment.Desc, time.Now()).Scan(&title)
	return err
}

func (r *SegmentsRepo) GetByTitle(ctx context.Context, title string) (*repository.Segment, error) {
	var segment repository.Segment
	err := r.db.Get(ctx, &segment, "SELECT title, desc, created_at, updated_at FROM segments WHERE title = $1", title)
...
func (r *SegmentsRepo) List(ctx context.Context) ([]*repository.Segment, error) {
	...
	err := r.db.Select(ctx, &segments, "SELECT title, desc, created_at, updated_at FROM segments")
	return segments, err
}

func (r *SegmentsRepo) Update(ctx context.Context, segment *repository.Segment, title string) (bool, error) {
	result, err := r.db.Exec(ctx,
		"UPDATE segments SET title = $1, desc = $2, updated_at = $3 WHERE title = $4", segment.Title, segment.Desc, time.Now(), title)
